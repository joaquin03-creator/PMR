...
import { runOhioXmlSelfTest } from '../lib/ohioSchemaLock';
...
export default function Reports({ profile }: { profile: any }) {
  ...
  useEffect(() => {
    ...
    const autoGenerateDateParam = location.state?.autoGenerateDate;
    if (autoGenerateDateParam) {
      setAutoGenerateDate(autoGenerateDateParam);
    }
  ...
  const [schemaLockViolations, setSchemaLockViolations] = useState<string[]>([]);
  const [autoGenerateDate, setAutoGenerateDate] = useState<string | null>(null);
  ...
  // Drawer-close handoff: auto-generate + auto-validate a specific day's Ohio
  // report when navigated here with location.state.autoGenerateDate (see
  // CashDrawer.tsx's post-close prompt). Fetches independently of the
  // complianceDate picker so it doesn't depend on that effect's timing.
  useEffect(() => {
    if (!autoGenerateDate) return;
    let cancelled = false;

    (async () => {
      try {
        const startOfDay = new Date(`${autoGenerateDate}T00:00:00`);
        const endOfDay = new Date(`${autoGenerateDate}T23:59:59.999`);
        const q = query(
        ...
        const completed = tickets.filter(t => (t.status || 'completed') === 'completed');
        if (cancelled) return;

        setComplianceDate(autoGenerateDate);
        setComplianceTickets(completed);
        setSelectedXmlTickets(completed.map(t => t.id));
        setSchemaLockViolations([]);

        const xml = await handleGenerateXml(completed);
        if (cancelled) return;

        const violations = runOhioXmlSelfTest(xml, completed.length);
        setSchemaLockViolations(violations);
        setNotification(
          violations.length > 0
            ? {
                type: 'error',
                message: `Auto-generated Ohio report for ${autoGenerateDate} failed the schema lock (${violations.length} issue${violations.length === 1 ? '' : 's'}). Review before downloading.`
              }
            : {
                type: 'success',
                message: `Auto-generated and validated the Ohio report for ${autoGenerateDate}. Ready to review and download.`
              }
        );
      } catch (e) {
        console.error('Auto-generate Ohio report failed:', e);
        if (!cancelled) {
          setNotification({ type: 'error', message: 'Could not auto-generate the Ohio report. Please generate it manually below.' });
        }
      } finally {
        if (!cancelled) setAutoGenerateDate(null);
      }
    })();
  ...
  const handleDownloadXml = () => {
    ...
    // OHIO SCHEMA LOCK — blocking gate. This only ever runs at the manager's
    // deliberate generate/download step in Reports; it can never interrupt
    // a ticket in progress. See src/lib/ohioSchemaLock.ts.
    const violations = runOhioXmlSelfTest(generatedXml, selectedXmlTickets.length);
    if (violations.length > 0) {
      setSchemaLockViolations(violations);
      setNotification({
        type: 'error',
        message: `Download blocked: generated XML failed the Ohio schema lock (${violations.length} issue${violations.length === 1 ? '' : 's'}). See details below.`
      });
      return;
    }
    setSchemaLockViolations([]);
                      ...
                      <div className="space-y-3">
                        ...
                        {generatedXml && (
                          <>
                            ...
                            {schemaLockViolations.length > 0 && (
                              <div className="p-4 bg-red-950/60 border border-red-500/40 rounded-2xl space-y-2">
                                <div className="flex items-center gap-2 text-red-300 font-black text-xs uppercase tracking-widest">
                                  <AlertCircle className="w-4 h-4" />
                                  Schema Lock — Download Blocked
                                </div>
                                <ul className="space-y-1 text-xs text-red-200 font-medium list-disc list-inside">
                                  {schemaLockViolations.map((v, i) => (
                                    <li key={i}>{v}</li>
                                  ))}
                                </ul>
                              </div>
