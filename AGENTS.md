...
- **Three deployments currently exist — know which is which** (as of 2026-09-17):
  - **(a) AI Studio dev-preview link (in daily use today):** `https://ais-dev-fiapso7rtkpidrzgaot6nx-163709909409.us-east1.run.app/` — runs inside a Google-managed GCP project the owner does NOT control; no backup, no custom domain. This is what staff actually log into every day.
...
- [ ] **[DECISION NEEDED] Production cutover is unresolved** — the yard currently runs daily on the AI Studio dev-preview link (`ais-dev-...run.app`), which lives in a Google-managed project the owner does NOT control (no backup, no custom domain). A real owned production deployment now exists in the owner's own GCP project via this repo's deploy path (see §2). A deliberate, planned cutover from the AI Studio link to the owned deployment is needed — **must not happen accidentally.**
